...
async function query(filterBy) {
    try {
        return await storageService.query(ENDPOINT, filterBy)
    }
    catch (err) {
        console.error('Cannot query books', err);
    }
}
...
async function save(book) {
    try {
        return book._id
            ? await storageService.put(ENDPOINT, book)
            : await storageService.post(ENDPOINT, book)
    }
    catch (err) {
        console.error('Cannot save book from book service', err);
    }
}
...
async function _createManyBooks() {
  try {
